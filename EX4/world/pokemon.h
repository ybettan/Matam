...
class Pokemon {
	...
	std::string species;
	int level;						// level > 0
	double hp,cp;					// 0 <= hp <= 100 , cp > 0
	std::set<PokemonType> types;



  // Computes the the hit_power and sum of types for boath pokemons.
  //
  // @param pokemon1_hit_power the hit power of this.
  // @param pokemon2_hit_power the hit power of rhs.
  // @param pokemon1_types_sum the types sum of this.
  // @param pokemon2_types_sum the types sum of rhs.
  static void strenghtParams(	const Pokemon& pokemon1,
  ...
  // returns the equivalent string for a specific PokemonType.
  //
  // @param type the enum value of PokemonType.
  static std::string pokemonType2str(PokemonType type);
