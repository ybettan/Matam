...
class PokemonGo {
	...
	World* world;
	vector<Placment> placments;
	// each element in placment is String(trainer location) + Trainer

	//return wether or not atrainer name alredy exist.
	bool IsTrainerNameAlredyExist(const string name) const;

	//return a trainer aplying to the given name.
	Trainer& GetTrainer(const string name) const;

	//return the name of the current location of a trainer given his name
	std::string& GetCurrentLocationName(const std::string& trainer_name);
 ...
 public:
  ...
  PokemonGo(World* world);
  
  // Disable copy and assignment.
  PokemonGo(const PokemonGo&) = delete;
  PokemonGo& operator=(const PokemonGo& pg) = delete;
