...
//-----------------------------------------------------------------------------
//build an array of pokemons including copies of all trainers pokemons and
//proffessor pokemons
static Pokemon* buildPokemonArrayFromTrainer(PokemonTrainer trainer) {
	...
	for (int i = 0 ; i < trainer->local_counter ; i++) {
		...
		if (total_array[i] == NULL) {		/////////////if failed meed to destroy all prvious pokemons and free array and return NULL - DONE
			destroyPokemonArray(total_array,i);
			return NULL;
		}
	}
	for (int i = 0 ; i < trainer->remote_counter ; i++) {
		total_array[trainer->local_counter + i] = /////////////if failed meed to destroy all prvious pokemons and free array and return NULL - DONE
						pokemonCopy(trainer->proffessor_pokemons_array[i]);
		if (total_array[trainer->local_counter + i] == NULL) {
			destroyPokemonArray(total_array,trainer->local_counter + i);
			return NULL;
		}
	}
	return total_array;
...
//-----------------------------------------------------------------------------
//remove pokemon at place [index] from array and shift left all pokemons to
//																	its right
static void removePokemonFromArray(Pokemon* array,int* size,int index) {
	...
	pokemonDestroy(array[index]);
	for (int i = index + 1 ; i < *size ; i++) {/////////////////////////first need to desrroy pokemon - DONE
		array[i-1] = array[i];
	}
	array[*size - 1] = NULL;
	*size = *size - 1;
...
//----Errors:----
//return POKEMON_TRAINER_OUT_OF_MEM if my_malloc failed
...
static PokemonTrainerResult sortPokemonsArray(Pokemon* pokemons_array,
	...
	for (int i = 0 ; i < size ; i++) {
		pokemons_array_copy[i] = pokemonCopy(pokemons_array[i]);//////if pokemon copy returned NULL needs to destroy all previous pokemons - DONE
		if (pokemons_array_copy[i] == NULL) {
			destroyPokemonArray(pokemons_array_copy,i);
...
//	free(pokemons_array_copy);
	destroyPokemonArray(pokemons_array_copy,pokemons_array_copy_size);///////////////after fixing removePokemonFromArray its ok - DONE
...
//return POKEMON_TRAINER_SUCCESS otherwise
static PokemonTrainerResult trainerReinsertPokemons(PokemonTrainer trainer,
	...
	for (int i= 0 ; i < pokemons_array_size ; i++) {
		if (trainer->local_counter < trainer->max_pokemons_local) {
			...
			if (trainer->trainer_pokemons_array[i] == NULL) {
				destroyPokemonArray(trainer->trainer_pokemons_array,i);
				return POKEMON_TRAINER_OUT_OF_MEM;
			}
			...
			if (trainer->proffessor_pokemons_array[i - trainer->local_counter]
												   	   	   	   	   == NULL) {
				destroyPokemonArray(trainer->proffessor_pokemons_array,i);
				return POKEMON_TRAINER_OUT_OF_MEM;
			}
		}
	}
...
//-----Errors:-----
//return POKEMON_TRAINER_OUT_OF_MEM for any file action or my_malloc failure
//retrun POKEMON_TRAINER_SUCCESS othewise
...
//	int flag = 0;
	for (int i = 1 ; i < trainer->local_counter ; i++) {
		...
		if (trainer_copy->trainer_pokemons_array[i] == NULL) {
			return NULL; ///destroy trainer??
//			flag = 1;
		}
	}
	for (int i = 0 ; i < trainer->remote_counter ; i++) {
		(trainer_copy->proffessor_pokemons_array)[i] =
						pokemonCopy((trainer->proffessor_pokemons_array)[i]);
		if (trainer_copy->proffessor_pokemons_array[i] == NULL) {
			return NULL; ///destroy trainer??
//			flag = 1;
		}
	}
...
PokemonTrainerResult pokemonTrainerAddPokemon(PokemonTrainer trainer,
	...
	if (trainer->local_counter == trainer->max_pokemons_local) {
		...
		return POKEMON_TRAINER_PARTY_FULL;///////////////////////////////destroy pokemon_copy - ALREDY HAD
	}else{
...
PokemonTrainerResult pokemonTrainerDepositPokemon(
	...
	(trainer->proffessor_pokemons_array)[trainer->remote_counter] = pokemon;
...
PokemonTrainerResult pokemonTrainerMakeMostRankedParty(PokemonTrainer trainer) {
	...
	if (result == POKEMON_TRAINER_OUT_OF_MEM) {
		...
		return POKEMON_TRAINER_OUT_OF_MEM;/////////needs to destroy each pokemon in array and free pokemons_array - DONE
	}

	resetTrainer(trainer);
	result = trainerReinsertPokemons(trainer,pokemons_array,pokemons_array_size);////////////what about the proffesor?? - DO ALSO FOR PROFFESSOR - ITS FIND
	destroyPokemonArray(pokemons_array,pokemons_array_size);

	if (result == POKEMON_TRAINER_OUT_OF_MEM)	return POKEMON_TRAINER_OUT_OF_MEM;
