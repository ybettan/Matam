...
#define TO_HIGHER_BOUNT_WHEN_DIVIDED_BY_TWO(num)	 num/2 + num % 2
...
//------------------------------------------------------------------------------
void Trainer::affectScores(const Trainer& attacker,const Trainer& victim) {
	affectScore(attacker.team,2,ADD);
	affectScore(victim.team,1,SUB);
}
...
bool Trainer::attack(Trainer& victim) {
	...
	if (*this == victim) {
		(winByTeam(victim))?affectScores(*this,victim):affectScores(victim,*this);
		return winByTeam(victim);
	}
